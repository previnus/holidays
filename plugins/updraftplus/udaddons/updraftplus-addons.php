...
class UpdraftPlusAddons2 {
	...
	public function __construct($slug, $url) {
		...
		if (is_readable(UPDRAFTPLUS_DIR.'/vendor/yahnis-elsts/plugin-update-checker/plugin-update-checker.php')) {
			...
			require_once(UPDRAFTPLUS_DIR.'/vendor/yahnis-elsts/plugin-update-checker/plugin-update-checker.php');
	...
	public function puc_check_now($shouldcheck, $lastcheck, $checkperiod) {
		...
		if (
			true !== $shouldcheck // ... no check was being made anyway
			|| empty($this->plug_updatechecker) // ... or if we don't seem to be able to check anything (e.g. no credentials)
			|| 0 == $lastcheck // ... or no check has ever been made before
			|| (is_array($wp_current_filter) && in_array('load-update-core.php', $wp_current_filter)) // ... or if we're on the 'updates' page
			|| !defined('DOING_CRON') // ... or if the check isn't a background one
		) return $shouldcheck;
